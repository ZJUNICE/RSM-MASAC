...
class SACContinuous:
    def __init__(self, state_dim, hidden_dim, action_dim, action_bound,
        ...
        self.policy_advantage = 0.0 
        self.trust = torch.tensor(0.5, dtype=torch.float).to(self.device)
    ...
    def return_param(self, actor_param):
    ...
    def get_policy_adv(self, model, batch_size, gamma):
        ...
        with torch.no_grad():
            ...
            policy_adv = torch.tanh((pi_tilde_prob - pi_self_prob) / (off_actions_prob + 1e-5)) * Q_value + self.log_alpha.exp() * entropy_minus
    ...
    def get_metric(self, model, states, actions, Coff):
        ...
        if appr:
            ...
            beta = torch.sqrt((2 * torch.sqrt(self.policy_advantage) / Coff * (param_div_vector.t() @ FIM @ param_div_vector))).squeeze()
            ...
            del actor_loss
            FIM /= batch
            beta = torch.sqrt((2 * torch.sqrt(self.policy_advantage).to(dtype=torch.float16) / Coff.to(dtype=torch.float16) * (param_div_vector.t() @ FIM @ param_div_vector))).squeeze()
            beta = beta.clone().detach()
            beta = beta.to(dtype=torch.float32)

        return beta
              
    def normalized_policy_adv(self):
        self.policy_advantage = self.policy_advantage * 1e2
...
def train(actor_lr, critic_lr, num_episodes, tau, batch_size, comm_interval, test_interval, repeat, cuda, segments, replica, max_step, radius, head, fine_tuning, checkpoint, loadfile, loss_rate, testOnly):
    ...
    writer = SummaryWriter(head + "rewards")
    checkpoint_filepath = head + "checkpoint.pth"
            ...
            for i_episode in range(int(num_episodes/10)): 
                ...
                if testOnly is False:     
                    while not done:
                        ...
                        if agents[0].replay_buffer.size() > 5 * batch_size : 
